...
struct ReceiverCtx {
  ...
  ReceiverCtx() : started(false), oldPacketsNum(0), packetsNum(0) {
    dynamicBufferInit(&buffer, 65536);
  }
};
                 ...
                 void *arg)
{
  ...
  aioReadMsg(base, socket, &ctx->buffer, 0, test_readcb, ctx);
}
                       ...
                       void *arg)
{
  ...
  if (status == aosSuccess) {
    ...
    if (ctx->packetsNum % ctx->config->groupSize == 0)
      ctx->endPt = getTimeMark();
    aioReadMsg(base, socket, &ctx->buffer, 1000000, test_readcb_timer, ctx);
  } else {
                          ...
                          void *arg)
{
  ...
  if (status == aosSuccess) {
    ...
    if (ctx->packetsNum % ctx->config->groupSize == 0)
      ctx->endPt = getTimeMark();
    aioReadMsg(base, socket, &ctx->buffer, 1000000, test_readcb_timer_rt, ctx);
  } else {
    if (!ctx->started || ctx->oldPacketsNum != ctx->packetsNum) {
      ctx->oldPacketsNum = ctx->packetsNum;
      aioReadMsg(base, socket, &ctx->buffer, 1000000, test_readcb_timer_rt, ctx);
...
void *test_aio_receiver(void *arg)
{
  ...
  switch (ctx->type) {
    case aioReceiverAsync :
      aioReadMsg(ctx->base, ctx->server, &ctx->buffer, 0, test_readcb, ctx);
      break;
    case aioReceiverAsyncTimer :
      aioReadMsg(ctx->base, ctx->server, &ctx->buffer, 1000000, test_readcb_timer, ctx);
      break;      
    case aioReceiverAsyncRT :
      fprintf(stderr, " * WARNING: asyncio-0.3 not support UDP receiving with RT timeouts\n");      
      aioReadMsg(ctx->base, ctx->server, &ctx->buffer, 1000000, test_readcb_timer_rt, ctx);
      break;
    default :
...
//   char msg[65536];
  ...
  for (;;) {
    for (unsigned i = 0; i < receiverCtx->config->groupSize; i++) {
      ssize_t result = ioReadMsg(receiverCtx->base, receiverCtx->server, &receiverCtx->buffer, 1000000);
      if (result == -1)
        return 0;      
...
{
  if (receiverTy != aioReceiverBlocking && receiverThreads > 1) {
    fprintf(stderr, "* WARNING: asyncio-0.3 don't support multi-threading, use asyncio-0.4 or higher\n");
    return;
  }
